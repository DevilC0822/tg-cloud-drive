...
import { formatDurationShort, historyDirectionLabel, historyStatusColor, historyStatusLabel } from '@/components/transfer/transferUtils';
...
export function HistoryFileItem({ item, onRequestRemove }: HistoryFileItemProps) {
  ...
  return (
    <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3 border-t border-neutral-200/80 px-4 py-3 first:border-t-0 dark:border-neutral-700/80 md:grid-cols-[minmax(0,1fr)_110px_90px_90px_180px_auto] md:items-center">
      <div className="min-w-0">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            ...
            {item.error ? <div className="mt-1 truncate text-xs text-red-600 dark:text-red-300">{item.error}</div> : null}
            <div className="mt-2 flex flex-wrap items-center gap-1.5 md:hidden">
              ...
              <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${historyStatusColor(item.status)}`}>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-neutral-500 dark:text-neutral-400 md:hidden">
      ...
      <div className="hidden text-xs text-neutral-700 dark:text-neutral-200 md:block">{formatFileSize(item.size)}</div>
      <div className="hidden text-xs text-neutral-700 dark:text-neutral-200 md:block">{formatDurationShort(duration)}</div>
      <div className="hidden md:block">
        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${historyStatusColor(item.status)}`}>
          {historyStatusLabel(item.status)}
        </span>
      </div>
      <div className="hidden text-xs text-neutral-500 dark:text-neutral-400 md:block">{finishedAtText}</div>
